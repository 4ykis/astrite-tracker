...
import { deleteSpendEntry, updateSpendEntry } from "./actions";
...
function EditRow({ entry, onDone }: { entry: SpendEntry; onDone: () => void }) {
  const [state, formAction, isPending] = useActionState(
    updateSpendEntry.bind(null, entry.id),
    undefined,
  );
      ...
      <form action={formAction} className="flex flex-col gap-2">
        <div className="flex flex-wrap items-center gap-2">
          ...
          <input
            ...
            required
            className="w-24 rounded-md border border-slate-700 bg-slate-950 px-2 py-1.5 text-sm text-slate-100 outline-none focus:border-amber-400"
          />
          <select
            name="category"
